        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          ...
          <div>
            <Link to="/" className="inline-flex items-center gap-3 mb-6">
              ...
              <div className="leading-none">
                ...
                <span className="block text-xs tracking-widest uppercase" style={{ color: "#555555" }}>Tree Company</span>
              </div>
            </Link>
            ...
            <div className="flex gap-4 mt-4">
              <a href={BUSINESS.social.facebook} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest" style={{ color: "#555555" }}>Facebook</a>
              <a href={BUSINESS.social.youtube} target="_blank" rel="noopener noreferrer" className="text-xs uppercase tracking-widest" style={{ color: "#555555" }}>YouTube</a>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="list-none m-0 p-0 flex flex-col gap-2.5">
              {SERVICES.map((s) => (
                <li key={s.id}>
                  <Link to={s.href} className="text-sm" style={{ color: "#555555" }}>{s.name}</Link>
                </li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="list-none m-0 p-0 flex flex-col gap-2.5 mb-10">
              {NAV_LINKS.map((link) => (
                <li key={link.href}>
                  <Link to={link.href} className="text-sm" style={{ color: "#555555" }}>{link.label}</Link>
                </li>
              ))}
              <li>
            ...
            <div className="p-4" style={{ border: "1px solid #1A1A1A", background: "#0A0A0A" }}>
              ...
              <p className="text-xs mt-1" style={{ color: "#555555" }}>7 Days a Week</p>
            </div>
          </div>
